...
SELECT artist_id , 
user_id , 
TO_TIMESTAMP(CAST(req_epoch AS NUMERIC)) $$ LANGUAGE SQL;
...
CREATE OR REPLACE FUNCTION insert_user_work (user_work_id TEXT, user_id TEXT, artist_id TEXT, req_epoch TEXT) RETURNS users_works_core
AS
$$
...
CREATE OR REPLACE FUNCTION check_signin (username TEXT, pwd TEXT) RETURNS checkable_signins_
AS
$$
...
CREATE OR REPLACE FUNCTION see_works (artist_id TEXT) RETURNS seeable_liked_works_ 
AS
$$
...
CREATE OR REPLACE FUNCTION see_watched_artists (user_id TEXT) RETURNS more_seeable_artists_
AS
$$
...
CREATE OR REPLACE FUNCTION view_user (TEXT) RETURNS users
AS
$$
SELECT $1
ORDER BY RANDOM() LIMIT 10;
...
CREATE OR REPLACE FUNCTION view_artist (TEXT) RETURNS artists
AS
$$
SELECT $1
ORDER BY RANDOM() LIMIT 10;
...
CREATE OR REPLACE FUNCTION ban_watcher (user_artist_id TEXT, req_epoch TEXT) RETURNS users_artists
AS
$$
...
CREATE OR REPLACE FUNCTION watch_artist (user_artist_id TEXT, user_id TEXT, artist_id TEXT, req_epoch TEXT) RETURNS users_artists
AS
$$
