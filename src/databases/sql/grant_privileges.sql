GRANT ALL
  ON simulation_003
  TO express_000;

GRANT ALL
  ON users_events
  TO express_000;

GRANT ALL
  ON simulation_001
  TO express_000;

GRANT ALL
  ON simulation_002
  TO express_000;

GRANT ALL
  ON simulation_000
  TO express_000;

GRANT ALL
  ON users_artists_events
  TO express_000;

GRANT ALL
  ON simulation_006
  TO express_000;

GRANT ALL
  ON simulation_005
  TO express_000;

GRANT ALL
  ON users_artists_core_buffer
  TO express_000;

GRANT ALL
  ON simulation_004
  TO express_000;

GRANT ALL
  ON users_core
  TO express_000;

GRANT ALL
  ON works_events
  TO express_000;

GRANT ALL
  ON users_works_events_buffer
  TO express_000;

GRANT ALL
  ON works_core_buffer
  TO express_000;

GRANT ALL
  ON artists_core_buffer
  TO express_000;

GRANT ALL
  ON requests_logs
  TO express_000;

GRANT ALL
  ON artists_core
  TO express_000;

GRANT ALL
  ON works_core
  TO express_000;

GRANT ALL
  ON users_artists_core
  TO express_000;

GRANT ALL
  ON users_artists_events_buffer
  TO express_000;

GRANT ALL
  ON works_events_buffer
  TO express_000;

GRANT ALL
  ON artists_events_buffer
  TO express_000;

GRANT ALL
  ON users_works_core_buffer
  TO express_000;

GRANT ALL
  ON users_works_events
  TO express_000;

GRANT ALL
  ON users_works_core
  TO express_000;

GRANT ALL
  ON artists_events
  TO express_000;

GRANT ALL
  ON users_events_buffer
  TO express_000;

GRANT ALL
  ON responses_logs
  TO express_000;

GRANT ALL
  ON users_core_buffer
  TO express_000;

GRANT ALL
  ON errors_logs
  TO express_000;

GRANT ALL
  ON full_logs
  TO express_000;

GRANT ALL
  ON users_keys
  TO express_000;

GRANT ALL
  ON artists_keys
  TO express_000;

GRANT ALL
  ON works_keys
  TO express_000;

GRANT ALL
  ON users_artists_keys
  TO express_000;

GRANT ALL
  ON users_works_keys
  TO express_000;

GRANT ALL
  ON non_deleted_users
  TO express_000;

GRANT ALL
  ON users_keys_without_deleted
  TO express_000;

GRANT ALL
  ON artists_keys_without_deleted
  TO express_000;

GRANT ALL
  ON non_deleted_artists
  TO express_000;

GRANT ALL
  ON works_keys_without_deleted
  TO express_000;

GRANT ALL
  ON non_deleted_works
  TO express_000;

GRANT ALL
  ON users_artists_keys_without_deleted
  TO express_000;

GRANT ALL
  ON users_works_keys_without_deleted
  TO express_000;

GRANT ALL
  ON non_withdrawn_works
  TO express_000;

GRANT ALL
  ON works_keys_without_withdrawn
  TO express_000;

GRANT ALL
  ON users_works_keys_without_withdrawn
  TO express_000;

GRANT ALL
  ON non_banned_users_artists
  TO express_000;

GRANT ALL
  ON users_artists_keys_without_banned
  TO express_000;

GRANT ALL
  ON users_works_keys_without_banned
  TO express_000;

GRANT ALL
  ON denormalized_users_with_pwd
  TO express_000;

GRANT ALL
  ON denormalized_users
  TO express_000;

GRANT ALL
  ON denormalized_artists
  TO express_000;

GRANT ALL
  ON denormalized_works
  TO express_000;

GRANT ALL
  ON checkable_signins
  TO express_000;

GRANT ALL
  ON checkable_signins_
  TO express_000;

GRANT ALL
  ON checkable_signups
  TO express_000;

GRANT ALL
  ON checkable_signups_
  TO express_000;

GRANT ALL
  ON seeable_watchers
  TO express_000;

GRANT ALL
  ON seeable_watchers_
  TO express_000;

GRANT ALL
  ON seeable_works
  TO express_000;

GRANT ALL
  ON seeable_works_
  TO express_000;

GRANT ALL
  ON seeable_artists
  TO express_000;

GRANT ALL
  ON seeable_artists_
  TO express_000;

GRANT ALL
  ON likable_works
  TO express_000;

GRANT ALL
  ON likable_works_
  TO express_000;

GRANT ALL
  ON more_artists
  TO express_000;

GRANT ALL
  ON more_artists_
  TO express_000;

GRANT ALL
  ON more_works
  TO express_000;

GRANT ALL
  ON more_works_
  TO express_000;

GRANT ALL
  ON viewable_users
  TO express_000;

GRANT ALL
  ON viewable_users_
  TO express_000;

GRANT ALL
  ON viewable_artists
  TO express_000;

GRANT ALL
  ON viewable_artists_
  TO express_000;

GRANT ALL
  ON viewable_works
  TO express_000;

GRANT ALL
  ON viewable_works_
  TO express_000;

